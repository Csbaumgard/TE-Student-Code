...
SELECT park_name, date_established FROM park ORDER BY date_established, park_name LIMIT 20;
...
SELECT (census_region || ', ' || state_name) AS census_region_state_name FROM state WHERE census_region IN ('West', 'Midwest') ORDER BY census_region_state_name ASC;
...
SELECT COUNT(population > 1000000) FROM city;
...
SELECT state_abbreviation, COUNT(city_name) FROM city GROUP BY state_abbreviation;
...
SELECT (SELECT state_name FROM state WHERE city.state_abbreviation = state.state_abbreviation),
COUNT(city_name) AS number_of_cities
FROM city
...
WHERE park.area = min_max_park_areas.largest_park_area
	OR park.area = min_max_park_areas.smallest_park_area
ORDER BY area DESC;
